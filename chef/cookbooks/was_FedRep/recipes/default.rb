...
#_________I_____________________________[ Changing WAS Process Execution to wasadmin ]
#execute ">>> Changing process execution" do
#  user "root"
#  cwd deploy_dir
#  command "/opt/IBM/WebSphere8/AppServer/profiles/CLM5/bin/startServer.sh -lang jython -f #{WAS_SCRIPTS_PROC_FILE}"
#  action :run
#end
...
execute ">>> Importing Certificates" do
  user "root"
  cwd deploy_dir
  command "/opt/IBM/WebSphere8/AppServer/profiles/CLM5/bin/wsadmin.sh -lang jython -f #{WAS_SCRIPTS_CERT_FILE}"
  action :run
end
...
execute ">>> Federating Repositories" do
  user "root"
  cwd deploy_dir
  command "/opt/IBM/WebSphere8/AppServer/profiles/CLM5/bin/wsadmin.sh -lang jython -f #{WAS_SCRIPTS_FEDERATE_FILE}"
  action :run
end
...
execute ">>> Updating JVS Custom Properties" do
  user "root"
  cwd deploy_dir
  command "/opt/IBM/WebSphere8/AppServer/profiles/CLM5/bin/wsadmin.sh -lang jython -f #{WAS_SCRIPTS_TUNE_FILE} /opt/IBM/JazzTeamServer"
  action :run
end
...
execute ">>> Deploying CLM WAR Files" do
  user "root"
  cwd deploy_dir
  command "/opt/IBM/WebSphere8/AppServer/profiles/CLM5/bin/wsadmin.sh -lang jython -f #{WAS_SCRIPTS_DPLY_FILE} /opt/IBM/JazzTeamServer"
  action :run
end
...
execute "changing process execution" do
  user "root"
  cwd deploy_dir
  command "#{WAS_BASE_INSTALLATION}/wsadmin.sh -language jython -f #{WAS_SCRIPTS_PROC_FILE}"
  action :run
end
...
execute ">>> Creating wasadmin user/group" do
  user "root"
  command "sh -c 'groupadd wasadmin ;useradd -m -g wasadmin wasadmin; chown -R wasadmin.wasadmin /opt/IBM/WebSphere8 /opt/IBM/CLM50'"
  action :run
end
