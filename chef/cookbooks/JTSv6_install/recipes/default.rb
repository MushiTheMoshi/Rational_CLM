...
## Get JTS v6 Installer
execute ">>> Getting JTS6 installer" do
  user "root"
  cwd deploy_dir
...
## Extract JTS v6 Installer
execute ">>> Unzipping  JTS6 Installer" do
  user "root"
  cwd deploy_dir
...
## Get JTS v6 Configuration Files
execute ">>> Getting JTS6 Configuration Files" do
  user "root"
  cwd deploy_dir
...
## Install JTS v6
execute ">>> Installing JTSv6" do
  user "root"
  cwd deploy_dir
  command "/opt/IBM/InstallationManager/eclipse/tools/imcl -input #{JTS_RESPONSE_FILE} -accessRights admin -silent -acceptLicense -log JTS_install.log"
  action :run
end
