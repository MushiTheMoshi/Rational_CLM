...
## Get QM v6 Installer
execute ">>> Getting QM6 installer" do
  user "root"
  cwd deploy_dir
...
## Extract QM v6 Installer
execute ">>> Unzipping  QM6 Installer" do
  user "root"
  cwd deploy_dir
...
## Get QM v6 Configuration Files
execute ">>> Getting QM6 Configuration Files" do
  user "root"
  cwd deploy_dir
...
## Install QM v6
execute ">>> Installing QMv6" do
  user "root"
  cwd deploy_dir
  command "/opt/IBM/InstallationManager/eclipse/tools/imcl -input #{JTS_RESPONSE_FILE} -accessRights admin -silent -acceptLicense -log QM_install.log"
  action :run
end
