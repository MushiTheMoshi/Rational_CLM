...
execute "Import Certificates" do
  user "root"
  cwd deploy_dir
  command "/opt/IBM/WebSphere8/AppServer/profiles/QM6/bin/wsadmin.sh -lang jython -f #{WAS_SCRIPTS_CERT_FILE}"
  action :run
end
...
execute "Enable Security" do
  user "root"
  cwd deploy_dir
  command "/opt/IBM/WebSphere8/AppServer/profiles/QM6/bin/wsadmin.sh -lang jython -f #{WAS_SCRIPTS_SECURITY_FILE}"
  action :run
end
...
execute "Configure LDAP" do
  user "root"
  cwd deploy_dir
  command "/opt/IBM/WebSphere8/AppServer/profiles/QM6/bin/wsadmin.sh -lang jython -f #{WAS_SCRIPTS_LDAP_FILE} CUSTOM bluepages.ibm.com 636 o=ibm.com dstacces@us.ibm.com '(&(emailaddress=%v)(objectclass=ePerson))' '(&(cn=%v)(|(objectclass=groupOfNames)(objectclass=groupOfUniqueNames)))' *:emailaddress *:cn 'ibm-allGroups:member;ibm-allGroups:uniqueMember'"
  action :run
end
...
execute "JVS Custom Properties" do
  user "root"
  cwd deploy_dir
  command "/opt/IBM/WebSphere8/AppServer/profiles/QM6/bin/wsadmin.sh -lang jython -f #{WAS_SCRIPTS_JVM_FILE} /opt/IBM/QM60/server/conf"
  action :run
end
...
execute "Modifying deploy warpath" do
  user "root"
  cwd deploy_dir
  command "sed -i 's|JazzTeamServer|QM60|g' #{WAS_SCRIPTS_JTS_FILE}"
  action :run
end
...
# Deploy QM .war files
execute "Deploy QM WAR Files" do
  user "root"
  cwd deploy_dir
  command "/opt/IBM/WebSphere8/AppServer/profiles/QM6/bin/wsadmin.sh -language jython -f #{WAS_SCRIPTS_JTS_FILE}"
  action :run
end
