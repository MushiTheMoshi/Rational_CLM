...
## Get RM v6 Installer
execute ">>> Getting RM6 installer" do
  user "root"
  cwd deploy_dir
...
## Extract RM v6 Installer
execute ">>> Unzipping  RM6 Installer" do
  user "root"
  cwd deploy_dir
...
## Get RM v6 Configuration Files
execute ">>> Getting RM6 Configuration Files" do
  user "root"
  cwd deploy_dir
...
## Install RM v6
execute ">>> Installing RMv6" do
  user "root"
  cwd deploy_dir
  command "/opt/IBM/InstallationManager/eclipse/tools/imcl -input #{JTS_RESPONSE_FILE} -accessRights admin -silent -acceptLicense -log RM_install.log"
  action :run
end
